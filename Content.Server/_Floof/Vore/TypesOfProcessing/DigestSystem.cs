...
public sealed class DigestSystem : EntitySystem
{
    ...
    /// </summary>
    internal void BuildDigestVerbs(EntityUid uid, VoreComponent comp, GetVerbsEvent<Verb> args){
        var user = args.User;
        ...
        foreach (var prey in container.ContainedEntities){
            ...
            if (_consentSystem.HasConsent(prey, "Digestable") && !digest.ActiveDigesting.Contains(prey)){
                args.Verbs.Add(new Verb
                {
                    ...
                    Act = () => TryDigest(user, prey, digest)
                });
            }
            ...
            //only shows up if the prey is currently being digested
                ...
                args.Verbs.Add(new Verb
                {
                    ...
                    Category = VerbCategory.Vore,
                    Act = () => StopDigest(user, prey, digest)
                });
            }
        }            
    }
    ...
    /// </summary>
    private void TryDigest(EntityUid pred, EntityUid prey, DigestComponent comp){
    ...
    /// </summary>
    private void StopDigest(EntityUid pred, EntityUid prey, DigestComponent comp){
